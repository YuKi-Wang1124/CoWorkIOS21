...
enum STMarketRequest: STRequest {
    ...
    var body: Data? {
        switch self {
        case .hots, .women, .men, .accessories, .all: return nil
        }
    }

    var method: String {
        switch self {
        case .hots, .women, .men, .accessories, .all: return STHTTPMethod.GET.rawValue
        }
    }
    ...
    var endPoint: String {
        ...
        case .all(let paging): return "/products/all?paging=\(paging)"
        }
    }
}
